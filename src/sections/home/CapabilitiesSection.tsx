...
export function CapabilitiesSection() {
  return (
    <section id="services" className="section-space bg-background relative overflow-hidden">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-24">
          {SERVICES_DATA.map((service, index) => (
            <motion.div
              ...
              className="group bg-surface/60 border border-border/40 p-8 md:p-10 rounded-sm hover:border-gold/30 transition-all duration-500 ease-out flex flex-col justify-between"
            >
              <div>
                ...
                <h3 className="text-heading-md font-heading font-extrabold text-foreground mb-4 group-hover:text-gold transition-colors duration-300">
                  {service.title}
                </h3>
              ...
              <Magnetic range={30} strength={0.3}>
                <Link
                  ...
                  className="text-xs font-heading font-extrabold tracking-widest text-foreground hover:text-gold uppercase py-2 transition-colors duration-300 inline-flex items-center gap-2"
                >
                  Explore Service
          ...
          <div className="flex flex-wrap gap-2.5 justify-center max-w-5xl mx-auto">
            {CAPABILITIES_TAGS.map((tag, idx) => (
              <motion.span
                ...
                className="text-[10px] md:text-xs font-heading tracking-widest uppercase font-bold px-4 py-2 bg-surface/40 border border-border/40 text-muted-foreground hover:border-gold/30 hover:text-gold transition-all duration-300 rounded-full select-none"
              >
                {tag}
              </motion.span>
            ))}
